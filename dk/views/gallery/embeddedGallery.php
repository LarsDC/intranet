...
// FUNCTIONS
	...
	function generateDirListing()
	{
		...
		if ($dir_handle == false)
		{
			...
			{
				echo "<li><a href='".$htmlpath."/".$fileArray[$i]."' target='_blank'><font size='6'>".strtr ($fileArray[$i], "_", " ")."</font></a><br />";
			}
	...
	for ($i = 0; $i < count($fileArray); $i++)
	{
		...
		//echo "\t\t<td class='phpGallery_td'>\n\t\t\t<a href='image.php?i=".$imagesFolder."/".$fileArray[$i]."'><img src='".$htmlpath."/".$thumbsFolder."/"."tn_".$fileArray[$i]."' alt='img' /></a>\n\t\t</td>\n";
                echo "\t\t<td class='phpGallery_td'>\n\t\t\t<a href='".$imagesFolder."/".$fileArray[$i]."' target='_blank'><img src='".$htmlpath."/".$thumbsFolder."/"."tn_".$fileArray[$i]."' alt='img' /></a>\n\t\t</td>\n";
